...
`POST /api/ingest/analytics-events` accepts exactly `{type, eventId}` for browser sessions, sticker generations, and shop clicks. UUID v4 IDs are required. Public quiz events are rejected; only the server's validated `/api/quiz/evaluate` success path records them. Firestore `create()` makes duplicate writes atomic, including concurrent requests and retries. Retry does not change the original timestamp.
...
4. **Shop:** Click a Shopee link on the static home page and on the React home or result page. Each click should add one redirect. Block `/api/ingest/analytics-events` in browser DevTools and click again: Shopee still opens. No purchase metric is created.
