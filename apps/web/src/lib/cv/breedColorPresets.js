...
export const BREED_COLOR_PRESETS = {
  ...
  "Siberian Husky": [
    ...
    { name: "All White/Cream", colors: { coat: "#E8DCC8" } }
  ]
};
...
// Roles are compared in a fixed priority: for breeds with both overcoat and
// undercoat layers, the dominant sampled color represents the outer fur and
// the secondary sampled color the inner/undercoat fur. For single-fur-layer
// breeds, many are patched with white that the "coat" layer doesn't cover
// (husky, German Shepherd, etc.), so the layer's real color might show up
// as the sample's secondary or darkest cluster rather than its most
// dominant one — every cluster is tried and whichever fits the preset best
// is used, instead of assuming it's always the dominant color.
function distanceToPreset(preset, sample) {
  ...
  if (colors.overcoat && colors.undercoat) {
    distance =
      colorDistance(sample.main, outerRgb) + colorDistance(sample.secondary, hexToRgb(colors.undercoat));
  } else {
    distance = Math.min(
      colorDistance(sample.main, outerRgb),
      colorDistance(sample.secondary, outerRgb),
      colorDistance(sample.dark, outerRgb)
    );
  }

  if (colors.eyes) {
    distance += colorDistance(sample.dark, hexToRgb(colors.eyes));
  }
